...
// Roof
const pyramid = new THREE.ConeGeometry(roofMeasurements.radius, roofMeasurements.height, roofMeasurements.segments)
...
const roof = new THREE.Mesh(pyramid, material)
...
door.position.z = houseMeasurements.width / 2 + 0.001
door.position.y = 1.1
house.add(door)
...
for (let i = 0; i < totalGraves; i++) {
	...
	const randomZ = Math.cos(angle) * radius

	// Mesh
	const grave = new THREE.Mesh(graveGeometry, graveMaterial)
	grave.position.x = randomX
	grave.position.y = Math.random() * 0.4
	grave.position.z = randomZ
...
// DoorLight
const doorLight = new THREE.PointLight('yellow', 1.7)
gui.add(doorLight, 'intensity').min(0.1).max(10).step(0.1).name('Door Light')
...
fontLoader.load('./fonts/helvetiker_regular.typeface.json', font => {
	...
	// Text
	const textGeometry = new TextGeometry(`Haunted House By Gopal`, {
        color:'#04343f',
		font: font,
		size: 0.5,
		depth: 0.2,
